...
class OrderControllerTest {
    ...
    class PayOrderTests {
        ...
        void payOrder_Success() throws Exception {
            ...
            when(orderService.payOrder(OrderMapper.toOrderPay(request))).thenReturn(order);
    ...
    private OrderDto.OrderCreateRequest createOrderRequestWithExceededQuantity() {
        ...
        for (int i = 0; i < MAX_ORDER_QUANTITY + 1; i++) {
            items.add(new CartItem(product, 20));
        }
    ...
    private Order createSampleOrder() {
        ...
        List<CartItem> items = List.of(new CartItem(new Product("test", PRODUCT_PRICE, PRODUCT_STOCK), 1));
