...
public class ProductIndexTest {
    ...
    @Test
    void testProductNameIndexPerformance() {
        // 인덱스 적용 전 쿼리 실행 계획
        List<Map<String, Object>> beforeIndex = getQueryPlan(
                "SELECT * FROM products WHERE name = 'Test Product'"
        );
        System.out.println("쿼리 실행 계획 (name 인덱스 적용 전):");
        printQueryPlan(beforeIndex);

        // 인덱스 생성
        jdbcTemplate.execute("CREATE INDEX idx_product_name ON products (name)");

        // 인덱스 적용 후 쿼리 실행 계획
        List<Map<String, Object>> afterIndex = getQueryPlan(
                "SELECT * FROM products WHERE name = 'Test Product'"
        );
        System.out.println("쿼리 실행 계획 (name 인덱스 적용 후):");
        printQueryPlan(afterIndex);

        // 인덱스 삭제
        jdbcTemplate.execute("DROP INDEX idx_product_name ON products");
